...
<body>
	...
	<div class="contenu">
		...
		<form action="<%=request.getContextPath()%>/repas" method="post">
			...
			<a href="<%=request.getContextPath()%>/repas"><input
		...
		<table align="center">
			...
			<tbody>
				...
				%>
				<tr>
					<td><%=repas.getDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy"))%></td>
					<td><%=repas.getHeure()%></td>

					<td>
						<%
							//Si le détail est déjà affiché, on enlève le détail
									if (String.valueOf(repas.getIdentifiant()).equals(request.getParameter("detail"))) {
						%> <a
						href="<%=request.getContextPath()%>/repas?dateFiltre=<%=dateFiltre%>">détail</a>
						<%
							} else {
						%> <a
						href="<%=request.getContextPath()%>/repas?detail=<%=repas.getIdentifiant()%>&dateFiltre=<%=dateFiltre%>">détail</a>
						<%
							}
						%>
		...
		<a href="<%=request.getContextPath()%>/ajoutrepas"><input
				type="button" value="Ajouter un nouveau repas" /></a> <a
